...
engine_names = {'namd':'CHARMM36', 'namd_scaled':'CHARMM36 + ECC', 'openmm':'AMOEBA'}
f = open('rshell.json')
...
for j, engine in enumerate(['namd', 'namd_scaled', 'openmm']):
    ax[j,0].set_ylabel(engine_names[engine])
    for i, anion in enumerate(['acet', 'esna', 'mso4']):
        if j == 0:
            ax[0, i].set_title(anion_names[anion])

        for k, cation in enumerate(['guan', 'imim', 'mamm']):
            if engine == 'namd':
            ...
            ax[j, i].plot(r_vals, Ka, label=cation_names[cation])
...
ax[0, 2].legend(handlelength=0.7, frameon=False, fontsize=14, loc='upper left')
plt.tight_layout()
plt.subplots_adjust(wspace=0.05, hspace=0.05)
ax[0, 0].set_ylim(0, 2.7)
ax[0, 0].set_xlim(3, 7.5)
fig.savefig(f'./images_v2_namd/Ka_vs_Rshell_cation_comparison.png', dpi=300)
