...
engine_ff = {'gromacs':'CHARMM36 NVT', 'namd':'CHARMM36', 'namd_scaled':'CHARMM36 + ECC', 'openmm':'AMOEBA'}
text = 'central_atoms'
    ...
    for data in ['pmf', 'gr']:
        ...
        for j, anion in enumerate(['acet', 'esna', 'mso4']):
            ...
            for i, cation in enumerate(['guan', 'imim', 'mamm']):
                ...
                for k, engine in enumerate(['namd', 'namd_scaled', 'openmm']): # enumerate(['gromacs', 'openmm']):
                    if engine == 'namd':
        ...
        ax[0, 0].legend(handlelength=0.7, frameon=False, fontsize=14, loc='lower right')
        ...
        if show_rshell:
            fig.savefig(f'./images_v2_namd/charmm_comparison_9_{data}_all_with_rshell.png', dpi=300)
        else:
            fig.savefig(f'./images_v2_namd/charmm_comparison_9_{data}_all.png', dpi=300)
