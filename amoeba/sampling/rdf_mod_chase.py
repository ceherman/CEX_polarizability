...
class InterRDF(AnalysisBase):
    ...
    def _prepare(self):
        ...
        # CH ADDITION
        self.results.frame_counts = np.zeros((self.n_frames, np.shape(count)[0]))
        # self.frame_volume = np.zeros(self.n_frames)
    ...
    def _single_frame(self):
        ...
        # CH ADDITION
        self.results.frame_counts[self._ts.frame,:] = count
        # self.frame_volume[self._ts.frame] = self._ts.volume
    ...
    def _conclude(self):
        ...
        # CH ADDITION
        frame_rdf = self.results.frame_counts / (density * vol)
        self.results.frame_rdf = frame_rdf
