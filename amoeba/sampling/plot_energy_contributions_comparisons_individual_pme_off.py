...
for k, (entry, label) in enumerate([('elect', 'Electrostatic'), ('vdw', 'VdW'),
                                    ('direct', 'Total direct interaction'),
                                    ('w_solv', 'Indirect solvent opposition')]):
    ...
    for j, anion in enumerate(['acet', 'esna', 'mso4']):
        ...
        for i, cation in enumerate(['guan', 'imim', 'mamm']):
            ...
            plot_with_seom(ax[j, i], df_ecc['r'], df_ecc[f'{entry}'], df_ecc[f'{entry}_seom'], label=f'CHARMM36+ECC')
    ...
    fig.savefig(f'./images_v2_namd/energy_contributions_comparison_individual_{entry}_pme_off.png', dpi=300)
