...
fig.supylabel(r'Energy at $r = \langle r_{\mathsf{PMF \; min}} \rangle_{\mathsf{FF}}$ [kcal/mol]', fontsize=35, x=0.05, y=0.57)
...
for d, (entry, label) in enumerate(data_names):
    ...
    for i, anion in enumerate(['acet', 'esna', 'mso4']):
        for j, cation in enumerate(['guan', 'imim', 'mamm']):
            ...
            for df, ff in [(df_amoeba, 'amoeba'), (df_charmm, 'charmm'), (df_ecc, 'ecc')]:
    ...
    for k, (ff, ff_label) in enumerate([('amoeba', 'AMOEBA'), ('charmm', 'CHARMM36'), ('ecc', 'CHARMM36 + ECC')]):
...
for i in range(2):
    ax[2, i].set_xticklabels(df_all.pair_name, rotation=40, ha='right', rotation_mode='anchor', fontsize=26)
    for j in range(3):
        ax[j, i].tick_params(axis='both',direction='in',length=6, bottom=True, top=True, left=True, right=True)

ax[1, 1].legend(loc='best', fontsize=20, frameon=False, handletextpad=0.8, handlelength=1)
plt.tight_layout()
plt.subplots_adjust(hspace=0.08, wspace=0.25)

fig.savefig(f'./images/energy_cont_bar_plots.eps', bbox_inches='tight')
fig.savefig(f'./images/energy_cont_bar_plots.pdf', bbox_inches='tight')
fig.savefig(f'./images/energy_cont_bar_plots.png', dpi=300, bbox_inches='tight')
