...
fig, ax = plt.subplots(5, 3, sharex=True, sharey='row')
fig.set_size_inches(16, 20)

for i in range(3):
    ax[4, i].set_xlabel(r'$r$ [\AA]')
    for j in range(5):
...
for k, (entry, label) in enumerate([('pmf', 'PMF'),
    ...
    for i, cation in enumerate(['guan', 'imim', 'mamm']):
        for j, anion in enumerate(['acet', 'esna', 'mso4']):
            ...
            df_amoeba = pd.read_csv(f'/home/chase/namd/ion_pairs/cgenff/brute_force_tip/{data_dir}/final_clean_pme_off.csv')
...
fig.savefig(f'./images/within_charmm_anion_comparison_all.pdf', bbox_inches='tight')
fig.savefig(f'./images/within_charmm_anion_comparison_all.png', dpi=300, bbox_inches='tight')
