...
data_names = [('pmf', 'PMF'), ('direct', r'$U_{Direct}$'), ('w_solv', r'$W_{Solv}$'), ('elect', 'Elect.'), ('vdw', 'VdW')]
...
for j, anion in enumerate(['acet', 'esna', 'mso4']):
    ...
    for i, cation in enumerate(['guan', 'imim', 'mamm']):
        ...
        df_charmm = pd.read_csv(f'/home/chase/namd/ion_pairs/cgenff/brute_force/{data_dir}/final_clean.csv')
        df_ecc = pd.read_csv(f'/home/chase/namd/ion_pairs/cgenff/brute_force_scaled/{data_dir}/final_clean.csv')
        ...
        k = len(data_names)
        plot_with_seom(ax[j, i], df_amoeba['r'], df_amoeba[f'polar'], df_amoeba[f'polar_seom'], color=f'C{k}', ls='--', label=f'Polarization AMOEBA')

# ax[0, 0].set_xlim(0, 16)
ax[0, 2].legend(handlelength=1, frameon=False, fontsize=16, bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.subplots_adjust(wspace=0.05, hspace=0.05)
fig.savefig(f'./images_v2_namd/energy_contributions_comparison_all.png', dpi=300)
