...
for k, engine in enumerate(['namd_tip', 'namd_tip_scaled', 'namd_tip_scaled_app', 'openmm']): # 'gromacs', 'namd_scaled',
    for i, cation in enumerate(['guan', 'imim', 'mamm']):
        for j, anion in enumerate(['acet', 'esna', 'mso4']):
            ...
            elif engine == 'namd_tip_scaled' or engine == 'namd_tip_scaled_app':
...
df_Ka.to_csv('./results_Ka_gr_tip.csv', index=False)
df_Kd.to_csv('./results_Kd_gr_tip.csv', index=False)
