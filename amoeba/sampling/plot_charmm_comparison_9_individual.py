...
for show_rshell in [True]:
    ...
    for k, engine in enumerate(['namd']): # , 'namd_scaled', 'openmm'
        for data in ['pmf', 'gr']:
            ...
            if show_rshell:
                fig.savefig(f'./images_v3_tip/charmm_comparison_9_{data}_individual_{engine}_with_rshell.png', dpi=300)
            else:
                fig.savefig(f'./images_v3_tip/charmm_comparison_9_{data}_individual_{engine}.png', dpi=300)
