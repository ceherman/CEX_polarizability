...
cation_names = {'guan':'Guanidinium', 'mamm':'Methyl-\nammonium', 'imim':'Imidazolium'}
...
for k in range(2):
    ...
    for m in range(3):
        ...
        ax[m, k].set_yticks([0, 45, 90, 135, 180])

for col_index, pair in enumerate([('mamm', 'acet'), ('mamm', 'esna')]):
    cation, anion = pair

    ax[0, col_index].set_title(f'{cation_names[cation]}\n{anion_names[anion]}', fontsize=25, loc='center', ha='center', va='bottom')
...
plt.savefig(f'./cation_hexmaps_selected_mamm.png', dpi=300, bbox_inches='tight')
