...
fig, axes = plt.subplots(nrows=3, ncols=9, sharex=True, sharey=True, constrained_layout=True)
...
for k in range(9):
    axes[2, k].set_xlabel(r'$r~[$\AA$]$', fontsize=fontsize)
    axes[2, k].set_xlim([2, 8])
    for m in range(3):
...
cbar = fig.colorbar(cm.ScalarMappable(norm=norm, cmap=cm.cool), ax=axes[:, 8], location='right', fraction=0.18)
...
plt.savefig('./heatmaps_anion_order.png', dpi=300, bbox_inches='tight')
plt.savefig('./heatmaps_anion_order.pdf', bbox_inches='tight')
# plt.savefig('./heatmaps.eps', transparent=False, bbox_inches='tight')
