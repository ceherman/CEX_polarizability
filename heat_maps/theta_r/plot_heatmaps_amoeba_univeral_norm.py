...
cbar.set_label('Universally normalized frequency', labelpad=10, fontsize=fontsize)
...
for l, anion in enumerate(anions):
    for n, cation in enumerate(cations):
        ...
        Zarr /= max(max_vals)
        Zarr[Zarr < 0.001] = np.nan
        ax[l, n].scatter(Xarr,Yarr,marker='s',c=Zarr, cmap='cool', vmin=0, vmax=max(max_vals))

# plt.subplots_adjust(wspace=0.05, hspace=0.1)
plt.savefig('./amoeba_heatmaps_universal_norm.eps', transparent=False, bbox_inches='tight')
plt.savefig('./amoeba_heatmaps_universal_norm.png', dpi=300, bbox_inches='tight')
