...
for j, anion in enumerate(['acet', 'esna', 'mso4']):
    ...
    for i, cation in enumerate(['guan', 'imim', 'mamm']):
        ...
        scalar = 0.75**2
        plot_with_seom(ax[j, i], df_charmm['r'], df_charmm[f'elect']*scalar, df_charmm[f'elect_seom']*scalar, label=r'$0.75^2 \; U_{q \; Elect}$')
        plot_with_seom(ax[j, i], df_ecc['r'], df_ecc[f'elect'], df_ecc[f'elect_seom'], label=r'$U_{0.75 q \; Elect}$')

ax[0, 0].set_xlim(2.8, 17)
ax[2, 0].legend(handlelength=1, frameon=False, fontsize=14, loc='lower right')
plt.tight_layout()
plt.subplots_adjust(wspace=0.05, hspace=0.05)
fig.savefig(f'./images/elect_contributions_ecc_comparison_pme_off.png', dpi=300)
