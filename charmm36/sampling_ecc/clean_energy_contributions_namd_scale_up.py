...
for j, anion in enumerate(['acet', 'esna', 'mso4']):
    for i, cation in enumerate(['guan', 'imim', 'mamm']):
        ...
        df_clean['direct'] = df_means['elect_mean']/(0.75**2) + df_means['vdw_mean']
        df_clean['direct_seom'] = np.sqrt((df_means['elect_seom']/(0.75**2))**2 + df_means['vdw_seom']**2)
        df_clean['w_solv'] = df_clean['pmf'] - df_clean['direct']
        df_clean['w_solv_seom'] = np.sqrt(df_clean['pmf_seom']**2 + df_clean['direct_seom']**2)

        df_clean['elect'] = df_means['elect_mean']/(0.75**2)
        df_clean['elect_seom'] = df_means['elect_seom']/(0.75**2)
        ...
        df_clean.to_csv(f'./{data_dir}/final_clean_pme_off_scaled_up.csv', index=False)
